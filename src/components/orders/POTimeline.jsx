...
function StuckBadge() {
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-red-50 border border-red-200 text-[10px] text-red-600 animate-pulse">
      ⚠ Stuck
    </span>
  );
}
...
function MilestoneRow({ state, isActive, isCompleted, isStuck, timestamp, isMobile }) {
    ...
    return (
      <div className="flex items-start gap-3 relative">
        <div className="flex flex-col items-center">
          ...
          <div className={`w-0.5 h-6 ${isCompleted ? "bg-emerald-200" : "bg-[#F0F0F0]"}`} />
        </div>
        <div className="pb-4">
          <div className={`text-xs font-medium ${isCompleted || isActive ? "text-[#0A0A0A]" : "text-[#9CA3AF]"}`}>
            {state.label}
          </div>
          {timestamp && (
  ...
  return (
    <div className="flex flex-col items-center gap-1 min-w-[72px]">
      ...
      <div className={`text-[10px] text-center leading-tight ${isCompleted || isActive ? "text-[#6B7280]" : "text-[#D4D4D4]"}`}>
        {state.label}
      </div>
      {timestamp && (
        <div className="text-[9px] text-[#D4D4D4]">{new Date(timestamp).toLocaleDateString()}</div>
      )}
      {isStuck && <StuckBadge />}
    </div>
  );
...
function LogisticsPicker({ poId, accessToken, onBooked }) {
  ...
  return (
    <div className="card p-4 mt-3 border-sky-200">
      <h4 className="text-xs font-semibold text-[#0A0A0A] mb-2">Book Logistics</h4>
      <div className="flex gap-2">
        <select value={carrier} onChange={(e) => setCarrier(e.target.value)} className="flex-1 px-3 py-2 bg-[#FAFAFA] border border-[#E5E5E5] rounded-lg text-xs text-[#0A0A0A]">
          <option value="">Select carrier…</option>
          {carriers.map((c) => <option key={c} value={c}>{c}</option>)}
        </select>
        <button onClick={book} disabled={!carrier || booking} className="px-4 py-2 bg-[#0A0A0A] text-white text-xs rounded-lg disabled:opacity-40">
...
export default function POTimeline({ poId, timeline: propTimeline, onRefresh }) {
  ...
  if (loading) return <div className="py-4 text-center text-xs text-[#9CA3AF]">Loading timeline…</div>;
    ...
    <div className="space-y-3">
      ...
      {timeline.carrier && (
        <div className="flex items-center gap-3 text-xs">
          <span className="text-[#0A0A0A]/40">Carrier:</span>
          <span className="text-[#0A0A0A]/70">{timeline.carrier}</span>
            ...
            return (
              <div key={state.key} className="flex items-center">
                ...
                {i < PO_STATES.length - 1 && (
                  <div className={`w-6 h-0.5 mt-[-18px] ${i < currentIdx ? "bg-emerald-200" : "bg-[#F0F0F0]"}`} />
                )}
              </div>
            );
          })}
