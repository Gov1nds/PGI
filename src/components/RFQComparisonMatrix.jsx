...
function Pill({ children, tone = "neutral" }) {
  const styles = {
    neutral: "bg-white/[0.04] text-white/55 border-white/[0.08]",
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-[#0f0f14] p-5 text-sm text-white/75">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-white">{workflowSummary.stageLabel}: {workflowSummary.description}</p>
              <p className="mt-1 text-xs text-white/35">{workflowSummary.breadcrumbs?.join(" → ")}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {onBackToVendor && (
                <button onClick={onBackToVendor} className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-2.5 text-sm font-medium text-white hover:bg-white/[0.08]">
                  Back to vendor match
                </button>
              )}
              {onContinue && (
                <button onClick={onContinue} className="rounded-xl bg-violet-500 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-400">
                  {workflowSummary.actionLabel || "Continue workflow"}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="rounded-2xl border border-white/[0.08] bg-[#0f0f14] p-5">
        <div className="grid grid-cols-1 gap-3 xl:grid-cols-6">
          <select
            ...
            className="rounded-xl border border-white/[0.08] bg-[#050816] px-4 py-3 text-sm text-white outline-none"
          >
