  ...
  return (
    <div className="card p-5 border-l-2 border-indigo-500/30 mb-6">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-semibold text-[#374151]/70 uppercase tracking-wider">AI Insight</span>
      </div>
      <p className="text-sm text-[#0A0A0A]/70 leading-6">{insight}</p>
    </div>
  );
}
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
      {kpis.slice(0, 4).map(([key, val]) => (
        <div key={key} className="kpi-card">
          ...
          <div className="text-[11px] text-[#6B7280] capitalize mt-0.5">{key.replace(/_/g, " ")}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="overflow-x-auto mt-4">
      <table className="w-full text-xs">
        <thead>
          <tr className="text-left text-[10px] uppercase tracking-wider text-[#0A0A0A]/25 border-b border-[#F0F0F0]">
        ...
        <tbody className="divide-y divide-white/[0.03]">
          {data.slice(0, 20).map((row, i) => (
            <tr key={i} className="hover:bg-[#FAFAFA]">
              {keys.map((k) => (
                <td key={k} className="py-2 pr-3 text-[#0A0A0A]/60">
    ...
    <div className="space-y-2 mt-4">
      {data.slice(0, 10).map((d, i) => (
        <div key={i} className="flex items-center gap-3">
          <span className="text-[11px] text-[#0A0A0A]/40 w-28 truncate capitalize">{d[labelKey]}</span>
          <div className="flex-1 h-5 bg-[#FAFAFA] rounded-md overflow-hidden">
            <div
              className="h-full bg-[#F5F5F5] rounded-md flex items-center pl-2 transition-all"
              style={{ width: `${max > 0 ? (d[valueKey] / max) * 100 : 0}%` }}
            >
              <span className="text-[10px] text-[#374151] font-medium whitespace-nowrap">
...
function ReportContent({ type, accessToken }) {
  ...
  if (loading) return <div className="py-8 text-center text-xs text-[#0A0A0A]/30">Generating report…</div>;
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mt-6">
        <button onClick={() => handleExport("pdf")} className="px-4 py-2 bg-[#F5F5F5] text-[#0A0A0A]/60 text-xs rounded-lg border border-[#E5E5E5] hover:bg-[#F0F0F0]">
          Export PDF
        </button>
        <button onClick={() => handleExport("xlsx")} className="px-4 py-2 bg-[#F5F5F5] text-[#0A0A0A]/60 text-xs rounded-lg border border-[#E5E5E5] hover:bg-[#F0F0F0]">
          Export XLSX
        </button>
      </div>
    </div>
