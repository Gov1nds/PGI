...
export default function Dashboard() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      setMetrics(metricsRes.status === "fulfilled" ? metricsRes.value || null : null);
      setSpendAnalytics(spendRes.status === "fulfilled" ? spendRes.value || null : null);
      setVendorAnalytics(vendorRes.status === "fulfilled" ? vendorRes.value || null : null);
      setCategoryAnalytics(categoryRes.status === "fulfilled" ? categoryRes.value || null : null);
      setTrendAnalytics(trendRes.status === "fulfilled" ? trendRes.value || null : null);
      setSavingsAnalytics(savingsRes.status === "fulfilled" ? savingsRes.value || null : null);
  ...
  const totalSpend = Number(spendAnalytics?.totals?.committed_spend ?? spendAnalytics?.committed_spend ?? m.total_spend ?? 0) || 0;
  const paidSpend = Number(spendAnalytics?.totals?.paid_spend ?? spendAnalytics?.paid_spend ?? 0) || 0;
  const savingsRealized = Number(spendAnalytics?.totals?.savings_realized ?? spendAnalytics?.savings_realized ?? 0) || 0;
  const monthlySpendSeries = normalizeSeries(
    firstArray(trendAnalytics, ["monthly_spend", "trends", "data", "series"]) ||
      firstArray(spendAnalytics, ["monthly_spend", "trends", "data", "series"]) ||
      [],
  ...
  const categorySeries = normalizeSeries(
    firstArray(categoryAnalytics, ["categories", "data", "series", "spend_by_category"]) ||
      firstArray(spendAnalytics, ["categories", "data", "series", "spend_by_category"]) ||
      [],
    ["category", "label", "name"],
    ["spend", "value", "amount", "total", "cost"]
  );
  ...
  const vendorSeries = normalizeSeries(
    firstArray(vendorAnalytics, ["vendors", "data", "series", "top_vendors", "spend_by_vendor"]) ||
      firstArray(spendAnalytics, ["vendors", "data", "series", "top_vendors", "spend_by_vendor"]) ||
      [],
  ...
  const activeRfqItems = safeArray(m.active_rfq_items).length
    ...
    : projects
        .filter((p) => ["rfq_pending", "rfq_sent", "quote_compare", "negotiation"].includes((p.workflow_stage || p.status || "").toLowerCase()))
        .slice(0, 4)
        .map((p, idx) => ({
                    ...
                    {notificationsOpen && (
                      <div className="absolute right-0 mt-3 w-[320px] rounded-2xl border border-white/[0.06] bg-[#0a0a12] p-3 shadow-2xl shadow-black/40">
                        ...
                        <div className="space-y-2 max-h-[380px] overflow-auto pr-1">
                          {notificationItems.length === 0 ? (
                            <p className="px-2 py-4 text-sm text-white/35">No notifications.</p>
                          ) : (
            ...
            {!loading && !error && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
                  ...
                  <Card title="Total spend" value={fmt(totalSpend)} hint={`Avg savings ${fmt(m.average_savings_percent || 0)}%`} />
                </div>
                ...
                <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-[1.65fr_0.95fr]">
                  ...
                  <div className="space-y-6">
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-start justify-between gap-3">
                                    <div>
                                      <p className="text-sm font-medium text-white">{getProjectName(project)}</p>
                                      <p className="mt-1 text-xs text-white/35">
                                        {project.analysis_status || "Normalized"} · {project.recommended_location || "No location"} · {project.total_parts || 0} parts
                                      </p>
                                    </div>
                                    <span
                                    ...
                                    >
                                      {(project.workflow_stage || project.status || "draft").replace(/_/g, " ")}
                                    </span>
                                  </div>
                      ...
                      >
                        <div className="space-y-3">
                          ...
                          {vendorSeries.length === 0 ? (
                            <p className="text-sm text-white/35">No vendor analytics available yet.</p>
                          ) : (
                                ...
                                <div key={`${vendor.label}-${idx}`} className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3">
                                  <div className="flex items-start justify-between gap-3">
                                    <div>
                                      ...
                                      <p className="mt-1 text-xs text-white/35">
                                        Match score {fmt(vendor.score || vendor.value || vendor.match_score || 0, 0)} · supplier fit
                                      </p>
                                    </div>
                                  ...
                                  <div className="mt-3 h-1.5 rounded-full bg-white/[0.04]">
                                    <div className="h-full rounded-full bg-gradient-to-r from-emerald-400 to-cyan-400" style={{ width: `${Math.max(10, Math.min(100, Number(vendor.value || vendor.score || 0)))}%` }} />
                                  </div>
                                </div>
                              ))}
                            </div>
                    ...
                    {/* Charts */}
                    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
                      <Panel title="BOM smart report summary" subtitle="Spend by category and obsolescence risk">
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                          <div className="h-[240px]">
                            <ResponsiveContainer width="100%" height="100%">
                              <PieChart>
                                <Pie
                                  data={categorySeries.length ? categorySeries : [{ label: "No data", value: 1 }]}
                                  dataKey="value"
                                  nameKey="label"
                                  innerRadius={58}
                                  outerRadius={85}
                                  paddingAngle={3}
                                >
                                  {(categorySeries.length ? categorySeries : [{ label: "No data", value: 1 }]).map((entry, idx) => (
                          ...
                          <div className="h-[240px]">
                            <ResponsiveContainer width="100%" height="100%">
                              <BarChart data={normalizeSeries(firstArray(spendAnalytics, ["obsolescence_risks", "risk_items", "flags", "series"]) || [], ["label", "name", "category"], ["value", "risk", "count", "score"])}>
                      ...
                      <Panel title="Estimated costing & spend insights" subtitle="Monthly spend and savings trend">
                        <div className="h-[260px]">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={monthlySpendSeries.length ? monthlySpendSeries : [{ label: "Jan", value: 0 }]}>
                              <CartesianGrid stroke="rgba(255,255,255,0.06)" vertical={false} />
                              <XAxis dataKey="label" tick={{ fill: "rgba(255,255,255,0.35)", fontSize: 11 }} />
                              <YAxis tick={{ fill: "rgba(255,255,255,0.35)", fontSize: 11 }} />
                              <Tooltip />
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3">
                          <Card title="Committed spend" value={fmt(totalSpend)} />
                    ...
                    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                      <Panel title="Quote comparison" subtitle="Side-by-side line item view">
                        {quoteComparisonRows.length === 0 ? (
                          ...
                          <div className="overflow-hidden rounded-2xl border border-white/[0.06]">
                            ...
                            <div className="divide-y divide-white/[0.05]">
                              {quoteComparisonRows.slice(0, 4).map((row, idx) => (
                                <div key={`${row.line_item || row.item || idx}`} className="grid grid-cols-5 gap-3 px-4 py-4 text-sm">
                                  <div className="col-span-2">
                                    <div className="font-medium text-white">{row.line_item || row.item || row.name || `Item ${idx + 1}`}</div>
                                    <div className="mt-1 text-xs text-white/35">{row.lead_time || row.leadTime || "—"} lead time</div>
                                  </div>
                                  <div className="text-white/70">{row.vendor_1 || row.vendor1 || row.price_1 || row.cost_1 || "—"}</div>
                                  <div className="text-white/70">{row.vendor_2 || row.vendor2 || row.price_2 || row.cost_2 || "—"}</div>
                                  <div className="text-white/70">{row.vendor_3 || row.vendor3 || row.price_3 || row.cost_3 || "—"}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      <Panel title="RFQ workflow" subtitle="Open sourcing requests, deadlines, and progress">
                              ...
                              <div key={item.id || item.name} className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                                ...
                                <div className="mt-3 h-2 rounded-full bg-white/[0.04]">
                                  <div
                                    ...
                                    style={{ width: `${progressValue(item.progress || 0)}%` }}
                                  />
                                </div>
                              </div>
                            ))
                    ...
                    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_0.9fr]">
                      <Panel title="Order center & logistics tracking" subtitle="PO, shipment, customs, delivery confirmation">
                        <div className="space-y-4">
                          {safeArray(m.order_items).length > 0 ? (
                              ...
                              <div key={order.id || idx} className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                                <div className="flex items-center justify-between gap-3">
                                  <div>
                                    ...
                                    <p className="mt-1 text-xs text-white/35">
                                      {order.vendor_name || order.vendor || "Vendor"} · {order.status || "Awaiting production"}
                                    </p>
                                  </div>
                                  <button
                                    onClick={goToWorkspace}
                            ...
                            <>
                              {safeArray(m.delayed_shipment_items).length === 0 ? (
                      ...
                      <Panel title="Collaboration hub" subtitle="Notifications, approvals, and negotiation updates">
                        <div className="space-y-3">
                              ...
                              <div key={item.id || idx} className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="text-sm font-medium text-white">{item.title || item.subject || item.message || "Update"}</p>
                                    <p className="mt-1 text-xs text-white/35">{item.message || item.reason || item.summary || "Procurement update"}</p>
                                  </div>
                                </div>
                              </div>
                            ))
                  ...
                  <div className="space-y-6">
                    ...
                    <Panel title="Spend by vendor" subtitle="Top supplier concentration">
                      <div className="h-[280px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={vendorSeries.length ? vendorSeries : [{ label: "No data", value: 1 }]}
                              dataKey="value"
                              nameKey="label"
                              innerRadius={55}
                              outerRadius={85}
                              paddingAngle={4}
                            >
                              {(vendorSeries.length ? vendorSeries : [{ label: "No data", value: 1 }]).map((entry, idx) => (
                                <Cell key={`vendor-${idx}`} fill={PIE_COLORS[idx % PIE_COLORS.length]} />
                              ))}
                            </Pie>
                            <Tooltip />
                          </PieChart>
                    ...
                    <Panel title="Monthly spend trend" subtitle="Last 12 months">
                      <div className="h-[260px]">
                        <ResponsiveContainer width="100%" height="100%">
                    ...
                    <Panel title="Savings trend" subtitle="Negotiated savings by period">
                      <div className="h-[220px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <LineChart data={savingsSeries.length ? savingsSeries : [{ label: "Jan", value: 0 }]}>
                            <CartesianGrid stroke="rgba(255,255,255,0.06)" vertical={false} />
                            <XAxis dataKey="label" tick={{ fill: "rgba(255,255,255,0.35)", fontSize: 11 }} />
                            <YAxis tick={{ fill: "rgba(255,255,255,0.35)", fontSize: 11 }} />
                            <Tooltip />
                ...
                <div className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
                  <Panel title="Quote comparison matrix" subtitle="Quick read of vendor pricing and timing">
                    <div className="overflow-hidden rounded-2xl border border-white/[0.06]">
                      ...
                      <div className="divide-y divide-white/[0.05]">
                        {(quoteComparisonRows.length ? quoteComparisonRows : [
                          { line_item: "1kΩ resistor", vendor_1: "$12.00", vendor_2: "$12.00", vendor_3: "$13.00", lead_time: "12m" },
                  ...
                  <Panel title="Action queue" subtitle="What needs attention now">
                    <div className="space-y-3">
                      {m.next_actions?.length === 0 ? (
                        <p className="text-sm text-white/35">No queue items.</p>
                      ) : (
                        m.next_actions.slice(0, 5).map((item) => (
                          <button
