...
function Check({ on }) {
  return (
    <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-[10px] ring-1 ${on ? "bg-sky-500/15 text-sky-300 ring-sky-500/25" : "bg-white/[0.03] text-white/20 ring-white/[0.08]"}`} aria-hidden="true">
      {on ? "✓" : "–"}
    </span>
  );
}
    ...
    <div>
      ...
      <Container className="py-16 md:py-20">
        ...
        <div className="grid gap-5 lg:grid-cols-3 mb-16">
          {pricingPlans.map((p) => (
            <div key={p.key} className={`relative overflow-hidden rounded-xl p-7 ring-1 transition-all duration-300 flex flex-col ${p.highlight ? "bg-gradient-to-br from-sky-500/10 to-sky-500/[0.02] ring-sky-500/25 shadow-lg shadow-sky-500/5" : "bg-white/[0.02] ring-white/[0.06] hover:ring-white/[0.12]"}`}>
              {p.highlight && (
