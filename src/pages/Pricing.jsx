...
function Check({ on }) {
  return (
    <span className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-[10px] ring-1 ${on ? "bg-sky-500/15 text-sky-300 ring-sky-500/25" : "bg-navy-800/50 text-white/20 ring-white/[0.08]"}`} aria-hidden="true">
      {on ? "✓" : "–"}
    </span>
  );
}
    ...
    <div>
        ...
        <Container>
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-sky-400 mb-4">
              <span className="w-6 h-px bg-sky-500/50" />Plans<span className="w-6 h-px bg-sky-500/50" />
            </div>
      ...
      <Container className="py-16 md:py-20">
        ...
        <div className="grid gap-5 lg:grid-cols-3 mb-16">
          {pricingPlans.map((p) => (
            <div key={p.key} className={`relative overflow-hidden rounded-2xl p-8 ring-1 shadow-glass transition-all duration-300 flex flex-col ${p.highlight ? "bg-gradient-to-br from-sky-500/10 to-sky-500/[0.02] ring-sky-500/25 shadow-lg shadow-sky-500/5" : "bg-navy-800/40 ring-white/[0.06] hover:ring-white/[0.12]"}`}>
              {p.highlight && (
                <div className="absolute right-4 top-4 rounded-full bg-sky-500/15 px-2.5 py-0.5 text-[10px] font-semibold text-sky-300 ring-1 ring-sky-500/25">POPULAR</div>
              )}
              <div>
                <div className="text-xl font-bold text-white">{p.name}</div>
                <div className="mt-2 text-2xl font-bold text-sky-400">{p.price}</div>
        ...
        {/* COMPARISON TABLE */}
        <RevealSection>
          <div className="rounded-xl bg-navy-800/40 ring-1 ring-white/[0.06] overflow-hidden mb-16">
            <div className="p-7">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/[0.06]">
                      <th className="text-left py-3 px-3 text-sm font-bold text-white/40">Feature</th>
                      <th className="text-center py-3 px-3 text-sm font-bold text-white/40">Prototype</th>
                      <th className="text-center py-3 px-3 text-sm font-semibold text-sky-400">Production</th>
                      <th className="text-center py-3 px-3 text-sm font-bold text-white/40">Scale</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pricingFeatures.map((f, i) => (
                      <tr key={i} className="border-b border-white/[0.03] hover:bg-navy-800/40 transition-colors">
          ...
          <div className="mb-16">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="rounded-xl bg-navy-800/40 ring-1 ring-white/[0.06] p-5">
        ...
        <div className="rounded-2xl bg-gradient-to-r from-sky-500/10 via-violet-500/5 to-cyan-500/5 p-10 text-center ring-1 ring-sky-500/10">
          ...
          <p className="mt-3 text-white/50 text-sm">Choose a plan and begin your manufacturing coordination journey</p>
