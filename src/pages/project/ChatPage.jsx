...
function ThreadSidebar({ threads, activeId, onSelect, onCreate }) {
  return (
    <div className="space-y-1">
      <button
        onClick={onCreate}
        className="w-full px-3 py-2 bg-[#0A0A0A] text-white text-xs rounded-lg font-medium hover:bg-[#1A1A1A] mb-2"
      >
        + New Thread
      </button>
      {threads.map((t) => (
        <button
          ...
          className={`w-full text-left p-3 rounded-xl border transition ${
            activeId === t.id
              ? "bg-indigo-900/10 border-[#D4D4D4]"
              : "card hover:border-[#E5E5E5]"
          }`}
        >
          <div className="text-sm text-[#0A0A0A] truncate">{t.title || "Thread"}</div>
          <div className="text-[10px] text-[#0A0A0A]/25 mt-0.5">
      ...
      {messages.map((m) => (
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            ...
            {m.message_type === "offer" && (
              <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-50 text-amber-600">
                Offer
              </span>
            )}
            {m._sending && (
              <span className="text-[9px] text-[#0A0A0A]/20">sending…</span>
            )}
            <span className="text-[9px] text-[#0A0A0A]/15 ml-auto">
          ...
          {m.message_type === "offer" && m.offer_payload_json && (
            <div className="mb-2 p-2 rounded-lg bg-[#FAFAFA] border border-[#F0F0F0]">
              <div className="text-xs text-[#0A0A0A]/50">
                ...
                {m.offer_payload_json.original_value && (
                  <span className="text-[#0A0A0A]/25 ml-2">
              ...
              {m.offer_status && (
                <div className={`text-[10px] mt-1 ${
                  m.offer_status === "accepted" ? "text-emerald-600" : m.offer_status === "rejected" ? "text-red-600" : "text-[#0A0A0A]/30"
                }`}>
          ...
          <div className="text-[#0A0A0A]/70">{m.content}</div>

          {/* Attachment */}
          {m.attachment_url && (
            <a href={m.attachment_url} target="_blank" rel="noopener" className="text-[11px] text-[#374151] mt-1 inline-block">
...
function OfferComposer({ onSubmit, onCancel }) {
  ...
  return (
    <div className="card p-4 border-amber-500/15 space-y-3">
      <h4 className="text-xs font-semibold text-[#0A0A0A] flex items-center gap-2">
        <span className="text-amber-600">💰</span> Make Offer
      </h4>
        ...
        {["price", "lead_time", "qty", "combined"].map((t) => (
          <button
            ...
            className={`px-2 py-1.5 rounded-lg text-[11px] border transition capitalize ${
              type === t ? "bg-amber-50 border-amber-500/20 text-amber-600" : "border-[#E5E5E5] text-[#0A0A0A]/30"
            }`}
          >
            {t.replace("_", " ")}
          </button>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="text-[10px] text-[#0A0A0A]/30 block mb-0.5">Proposed Value</label>
        ...
        <div>
          <label className="text-[10px] text-[#0A0A0A]/30 block mb-0.5">Original Value</label>
      ...
      <div className="flex gap-2">
        <button onClick={() => onSubmit({ offer_type: type, proposed_value: proposed, original_value: original, note })} className="px-4 py-2 bg-amber-600 text-[#0A0A0A] text-xs rounded-lg font-medium hover:bg-amber-500">
          Send Offer
        </button>
        <button onClick={onCancel} className="text-xs text-[#0A0A0A]/30 hover:text-[#0A0A0A]/60">Cancel</button>
      </div>
    </div>
  );
}
...
function MessageComposer({ onSend, onOffer }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-2">
        <select value={vis} onChange={(e) => setVis(e.target.value)} className="px-2 py-2 bg-[#FAFAFA] border border-[#E5E5E5] rounded-lg text-[11px] text-[#0A0A0A]">
        ...
        <input
          ...
          className="flex-1 px-3 py-2 bg-[#FAFAFA] border border-[#E5E5E5] rounded-lg text-sm text-[#0A0A0A] placeholder:text-[#9CA3AF] focus:outline-none focus:border-indigo-500/30"
        />
        <button onClick={send} className="px-4 py-2 bg-[#0A0A0A] text-white text-xs rounded-lg hover:bg-[#1A1A1A] font-medium">
          Send
        </button>
      </div>
      <div className="flex gap-2">
        <button onClick={() => setShowOffer(true)} className="text-[11px] text-amber-600/60 hover:text-amber-600 transition">
          💰 Make Offer
        </button>
        <button onClick={() => fileRef.current?.click()} className="text-[11px] text-[#0A0A0A]/30 hover:text-[#0A0A0A]/50 transition">
          📎 Attach File
        </button>
...
export default function ChatPage({ projectId }) {
  ...
  if (loading) return <div className="p-6 text-xs text-[#0A0A0A]/30">Loading chat…</div>;

  return (
    <div className="p-6">
      <h2 className="text-xl font-bold text-[#0A0A0A] mb-4">Chat</h2>
          ...
          {activeId ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-1.5 text-[10px]">
                <span className={`w-1.5 h-1.5 rounded-full ${wsRef.current?.connected ? "bg-emerald-500" : "bg-white/20"}`} />
                <span className="text-[#0A0A0A]/25">{wsRef.current?.connected ? "Connected" : "Connecting…"}</span>
              </div>
