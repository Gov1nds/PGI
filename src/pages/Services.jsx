    ...
    <div>
      <section className="border-b border-white/[0.08] bg-gradient-to-b from-[#09090b] to-[#0f0f14] py-16 md:py-20">
        <Container>
          <div className="text-center max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-white/70 mb-4">
              <span className="w-6 h-px bg-white/[0.04]0" />Services<span className="w-6 h-px bg-white/[0.04]0" />
            </div>
        ...
        <section className="border-b border-white/[0.08] py-16 md:py-20">
          <Container>
            <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
              {services.map((s, idx) => (
                <div key={idx} className="group rounded-xl bg-white/[0.04] p-5 ring-1 ring-white/[0.08] hover:ring-white/[0.1] transition-all duration-300 flex flex-col">
                  ...
                  <h3 className="text-base font-semibold text-white group-hover:text-white/70 transition-colors">{s.title}</h3>
                  <p className="mt-2 text-sm text-white/75 leading-relaxed">{s.desc}</p>
                  <div className="mt-4 pt-4 border-t border-white/[0.08]">
                    <p className="text-[11px] font-semibold uppercase tracking-wider text-white/70/70 mb-2.5">Key Benefits</p>
                    <ul className="space-y-1.5">
                      {s.benefits?.map((b, i) => (
                        <li key={i} className="text-xs text-white/80 flex items-start gap-2">
                          <span className="text-white/30 mt-0.5 text-[10px]">▸</span><span>{b}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <section className="border-b border-white/[0.08] py-16 md:py-20">
          <Container>
            <div className="text-center mb-12">
              <div className="text-xs font-semibold uppercase tracking-[0.15em] text-white/70 mb-3">Detailed Capabilities</div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={idx} className="rounded-xl bg-white/[0.04] ring-1 ring-white/[0.08] hover:ring-white/[0.08] p-6 transition-all duration-300">
                      ...
                      <ul className="grid md:grid-cols-2 gap-2">
                        {cap.items.map((item, i) => (
                          <li key={i} className="text-sm text-white/80 flex items-center gap-2">
                            <span className="w-1 h-1 rounded-full bg-white/[0.15] shrink-0" />{item}
                          </li>
                        ))}
                      </ul>
                    </div>
      ...
      <RevealSection>
        <section className="py-16 md:py-20">
          <Container>
            <div className="text-center mb-12">
              <div className="text-xs font-semibold uppercase tracking-[0.15em] text-white/70 mb-3">Our Process</div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {[
                ...
                <div key={idx} className="text-center p-4">
                  <div className="rounded-xl w-14 h-14 bg-white/[0.06] ring-1 ring-white/[0.1] flex items-center justify-center mx-auto mb-3">
                    <span className="text-xl font-bold text-white/70 font-mono">{p.step}</span>
                  </div>
      ...
      <section className="py-14 border-t border-white/[0.08]">
        <Container>
          <div className="rounded-2xl bg-gradient-to-r from-white/[0.03] to-white/[0.01] p-10 text-center ring-1 ring-white/[0.08]">
